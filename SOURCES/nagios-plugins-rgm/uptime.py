...
### CHANGES :
  ...
  * 1.0.0     30Aug18      Julien Dumarchey <jdumarchey@fr.scc.com>    Initial version
'''
...
def display_help():
    ...
    v = "   -v: Verbose.\n\n"
    e = "EXAMPLES:\n"
    e1a = "   -> Get Uptime for machine srv3 only if monitored data is not anterior at 4 minutes (4: default value). Warning alert if Uptime < 10 minutes. Critical alert if Uptime < 5 minutes. \n"
    e1b = "      python uptime.py -H srv3 -w 10 -c 5\n\n"
    e2a = "   -> Get Uptime for machine srv3 only if monitored data is not anterior at 2 minutes.\n"
    e2b = "      python uptime.py -H srv3 -w 10 -c 5 -t 2\n\n"
    e3a = "   -> Get Uptime for machine srv3 with Verbose mode enabled.\n"
    e3b = "      python uptime.py -H srv3 -w 10 -c 5 -v\n\n"
    e4a = "   -> Get Uptime for machine srv3 with Verbose mode enabled and only if monitored data is not anterior at 2 minutes. \n"
    e4b = "      python uptime.py -H srv3 -w 10 -c 5 -t 2 -v\n"
...
def get_uptime(plugin_hostname,data_validity,verbose):
    try:
        ...
        print("Error calling \"get_uptime\"...")
        sys.exit()

# Convert Uptime in milliseconds in a human-readable format -> Days:Hours:Minutes:Seconds:
def convert_uptime(plugin_hostname,data_validity,verbose):
    try:
        uptime = int(get_uptime(plugin_hostname,data_validity,verbose))
        ...
        print("Error calling \"convert_uptime\"...")
        sys.exit()

# Display Uptime (System Information + Performance Data) in a format compliant with RGM expectations:
def rgm_uptime_output(plugin_hostname,warning_treshold,critical_treshold,data_validity,verbose):
    try:
        # Get Uptime values:
        uptime_exists, days, hours, minutes, converted_uptime = convert_uptime(plugin_hostname,data_validity,verbose)
        ...
        print("Error calling \"rgm_uptime_output\"...")
        sys.exit(3)
...
def main_parse_script_command(argv):
    ...
    if (len(sys.argv)) > 10:
        ...
        data_validity = "4"
        verbose = "0"
        rgm_uptime_output(plugin_hostname,warning_treshold,critical_treshold,data_validity,verbose)
